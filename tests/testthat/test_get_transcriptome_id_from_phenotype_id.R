test_that("get_transcriptome_id_from_phenotype", {
  ...
  suppressMessages({
    ...
    found1 <- get_transcriptome_id_from_phenotype_id(
      phenotype_id = 1,
      seed_id = 1,
      triplestore = triplestore
    )
  
    found2 <- get_transcriptome_id_from_phenotype_id(
      phenotype_id = c(1, 2),
      seed_id = c(2,1),
    ...
    found3 <- get_transcriptome_id_from_phenotype_id(
      phenotype_id = c(1, 2),
      seed_id = c(2,1),
      transcriptome_seq = FALSE,
    ...
    expect_equal(colnames(found1), c("seed_id", "transcriptome_id", "phenotype_id"))
  
    expect_match(class(found2), "data.frame")
    expect_true(nrow(found2) > 0)
    expect_equal(colnames(found2), c("seed_id", "transcriptome_id", "phenotype_id"))
    expect_equal(found2$seed_id[1], "seed_2")
  
    expect_match(class(found3), "data.frame")
    expect_true(nrow(found3) > 0)
    expect_equal(colnames(found3), c("seed_id", "transcriptome_id", "transcriptome_pos", "phenotype_id"))
